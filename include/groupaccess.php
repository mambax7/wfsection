<?php 
// $Id: groupaccess.php,v 1.4 2004/08/13 12:48:54 phppp Exp $
//  ------------------------------------------------------------------------ //
...
function wfs_listGroups($grps = "-1")
{
    global $xoopsDB, $myts;
 ...
 * See docs for usage
 */
function wfs_checkAccess($grps, $time = -1, $message = '')
{
 ...
 * @return 
 */
function wfs_checkadminAccess($grps)
{
    global $xoopsUser, $xoopsModule;
...
function wfs_saveAccess($grps)
{
    if (is_array($grps)) $grps = implode(" ", $grps);
    return($grps);
}

function wfs_getGroupIda($grps)
{
    $ret = array();
