<?php
// $Id: mimetypes.php,v 1.0 2004/08/13 12:41:44 wfsection Exp $
//  ------------------------------------------------------------------------ //
...
function edit_mimetype($mime_id = 0)
{
    ...
    if ($mime_id != 0)
    {
        $query = "select * from " . $xoopsDB -> prefix('wfs_mimetypes') . " WHERE mime_id = $mime_id";
        $mime_arr = $xoopsDB -> fetchArray($xoopsDB -> query($query));
    } 
    $forminfo = ($mime_id == 0) ? _AM_WFS_MIME_CREATEF : _AM_WFS_MIME_MODIFYF;
    $sform = new XoopsThemeForm($forminfo , "op", xoops_getenv('PHP_SELF'));
    $sform -> addElement(new XoopsFormText(_AM_WFS_MIME_EXTF, 'mime_ext', 5, 60, $mime_arr['mime_ext']), true);
    $sform -> addElement(new XoopsFormText(_AM_WFS_MIME_NAMEF, 'mime_name', 50, 255, $mime_arr['mime_name']), true);
    $sform -> addElement(new XoopsFormTextArea(_AM_WFS_MIME_TYPEF, 'mime_type', $mime_arr['mime_types'], 7, 60));
    $madmin_radio = new XoopsFormRadioYN(_AM_WFS_MIME_ADMINF, 'mime_admin', intval($mime_arr['mime_admin']), ' ' . _YES . '', ' ' . _NO . '');
    $sform -> addElement($madmin_radio);
    $muser_radio = new XoopsFormRadioYN(_AM_WFS_MIME_USERF, 'mime_user', intval($mime_arr['mime_user']), ' ' . _YES . '', ' ' . _NO . '');
    ...
    if (!$mime_id)
    {
        $butt_create = new XoopsFormButton('', '', _AM_WFS_MIME_CREATE, 'submit');
        $butt_create -> setExtra('onclick="this.form.elements.op.value=\'save\'"');
        $button_tray -> addElement($butt_create);

        $butt_clear = new XoopsFormButton('', '', _AM_WFS_MIME_CLEAR, 'reset');
        $button_tray -> addElement($butt_clear);

        $butt_cancel = new XoopsFormButton('', '', _AM_WFS_MIME_CANCEL, 'button');
        $butt_cancel -> setExtra('onclick="history.go(-1)"');
        $button_tray -> addElement($butt_cancel);
    } 
    else
    {
        $butt_create = new XoopsFormButton('', '', _AM_WFS_MIME_MODIFY, 'submit');
        $butt_create -> setExtra('onclick="this.form.elements.op.value=\'save\'"');
        $button_tray -> addElement($butt_create);
    ...
    $iform = new XoopsThemeForm(_AM_WFS_MIME_FINDMIMETYPE, "op", xoops_getenv('PHP_SELF'));
    $iform -> addElement(new XoopsFormText(_AM_WFS_MIME_EXTFIND, 'fileext', 5, 60, ""), true);
    $button_tray = new XoopsFormElementTray('', '');
    $hidden = new XoopsFormHidden('op', 'openurl');
    $button_tray -> addElement($hidden);
    $butt_create = new XoopsFormButton('', '', _AM_WFS_MIME_FINDIT, 'submit');
...
switch ($op)
{
    ...
    case 'update';
        ...
        $query = "UPDATE " . $xoopsDB -> prefix('wfs_mimetypes') . " SET 
        ...
        redirect_header("mimetypes.php?start=" . $_GET['start'] . "", 0, _AM_WFS_MIME_MODIFIED);
        break;
    ...
    case 'save':
        ...
        if ($mime_id == 0)
        {
            $query = "INSERT INTO " . $xoopsDB -> prefix("wfs_mimetypes") . " 
        ...
        $dbupted = ($mime_id == 0) ? _AM_WFS_MIME_CREATED : _AM_WFS_MIME_MODIFIED;
    ...
    case 'saveall':
        ...
        redirect_header("mimetypes.php", 1, _AM_WFS_MIME_MODIFIED);
        break;
    ...
    case "delete":
        ...
        if ($confirm)
        {
            $sql = "DELETE FROM " . $xoopsDB -> prefix("wfs_mimetypes") . " WHERE mime_id = " . $_POST['mime_id'] . "";
            $result = $xoopsDB -> query($sql);

            if ($result)
            {
                redirect_header("mimetypes.php", 1, sprintf(_AM_WFS_MIME_MIMEDELETED, $_POST['mime_name']));
            } 
            else
            {
                $error = "" . _AM_WFS_EVENNEWS_DBERROR . ": <br /><br />" . $sql;
                trigger_error($error, E_USER_ERROR);
            } 
            exit();
        } 
        else
        {
            ...
            $result = $xoopsDB -> query("SELECT mime_id, mime_name FROM " . $xoopsDB -> prefix("wfs_mimetypes") . " WHERE mime_id = '$mime_id'");
            list($mime_id, $mime_name) = $xoopsDB -> fetchrow($result);
            xoops_cp_header();
            xoops_confirm(array('op' => 'delete', 'mime_id' => $mime_id, 'confirm' => 1, 'mime_name' => $mime_name), 'mimetypes.php', _AM_WFS_MIME_DELETETHIS . "<br /><br>" . $mime_name, _AM_WFS_MIME_DELETE);
            xoops_cp_footer();
        } 
        break;
    ...
    case 'edit':
        xoops_cp_header();
        wfs_admin_menu(_AM_WFS_MMIMETYPES);
    ...
    default:
        ...
        $query = "select * from " . $xoopsDB -> prefix('wfs_mimetypes') . " ORDER BY mime_ext";
        ...
        echo "
		<fieldset><legend style='font-weight: bold; color: #900;'>" . _AM_WFS_MIME_MODIFYF . "</legend>\n
		<div style='padding: 8px;'>" . _AM_WFS_MIME_INFOTEXT . "</div>\n
		</fieldset>\n
		";
        ...
        echo "
		<fieldset><legend style='font-weight: bold; color: #900;'>" . _AM_WFS_MIME_ADMINF . "</legend>\n
		<div style='padding: 8px;'>" . _AM_WFS_MIME_ADMINFINFO . "</div>\n
		";

        $query = "select mime_ext from " . $xoopsDB -> prefix('wfs_mimetypes') . " WHERE mime_admin = 1 ORDER by mime_ext";
        ...
        {
            echo "<div style='padding: 8px;'>" . _AM_WFS_MIME_NOMIMEINFO . "</div>";
        } 
        echo "
		</fieldset><br />\n
		<fieldset><legend style='font-weight: bold; color: #900;'>" . _AM_WFS_MIME_USERF . "</legend>\n
		<div style='padding: 8px;'>" . _AM_WFS_MIME_USERFINFO . "</div>\n
		";

        $query = "select mime_ext from " . $xoopsDB -> prefix('wfs_mimetypes') . " WHERE mime_user = 1 ORDER by mime_ext";
        $result = $xoopsDB -> query($query);
        $allowmimetypes = '';
        while ($mime_arr = $xoopsDB -> fetchArray($result))
        {
        ...
        $headingarray = array(_AM_WFS_MIME_ID, _AM_WFS_MIME_EXT,_AM_WFS_MIME_NAME, _AM_WFS_MIME_ADMIN, _AM_WFS_MIME_USER, _AM_WFS_MINDEX_ACTION);
        ...
        $page = ($mime_num > 20) ? _AM_WFS_MINDEX_PAGE : '';
